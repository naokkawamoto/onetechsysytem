/**
 * 請負見積（quote-create）の「詳細設定」タブ（#quote-items）の明細行初期値を localStorage で上書きし、
 * マスタ管理から JSON で取り込み・保存するためのユーティリティ。
 */
(function (global) {
    var STORAGE_KEY = "mmUkeoiQuoteItemsDefaults_v1";
    var DOC_VERSION = 1;
    ...
    function fetchBuiltinRowsFromQuoteCreateHtml(htmlUrl) {
    ...
    function applyRowsToLiveDom(rootSelector) {
        ...
        var root = doc.querySelector(rootSelector || "#quote-create");
        if (!root) return;
        ...
        for (var i = 0; i < n; i++) {
            ...
            if (kind && d.kind != null) {
                kind.value = String(d.kind);
            }
            if (scope && d.scope != null) {
                scope.value = String(d.scope);
            }
            if (md && d.md != null) {
                md.value = String(d.md);
            }
            if (note) note.value = d.note != null ? String(d.note) : "";
            if (isFlat) {
                if (am && d.amount != null) {
                    am.value = String(d.amount);
                }
        ...
        if (global.window && global.window.QuoteUkeoiCreateRecalc && typeof global.window.QuoteUkeoiCreateRecalc.recalcAll === "function") {
            global.window.QuoteUkeoiCreateRecalc.recalcAll();
        }
    }

    global.QuoteUkeoiItemsDefaults = {
        ...
        fetchBuiltinRowsFromQuoteCreateHtml: fetchBuiltinRowsFromQuoteCreateHtml,
